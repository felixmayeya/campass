//SQL
function sqlBtn(chart, smChart) {
    pageName = "INJECTION";
    btnUrl = '/api/injection.open';
    ...
    //swiper插件配置与初始化
    var paginationArr = Dynamic.analysis_sql_paginationArr
    if (swiper) {
        ...
        onSlideChangeStart: function(swiper) {
            ...
            if (swiper.activeIndex == 0 && chartTime[swiper.activeIndex] != currentChartTime) {
                var barChart = new chart.Chart($(".echarts_bar1"), {
                    type: 'barplot',
                    title: Dynamic.homeCharts_title32,
                    clickable: false
                })
                chartObj[0] = [barChart];
                ...
                var vBarChart = new chart.Chart($(".echarts_vBar1"), {
                    type: 'hbarplot',
                    title: Dynamic.homeCharts_title33
                })
                ...
                var vBarChart = new chart.Chart($(".echarts_vBar2"), {
                    type: 'hbarplot',
                    title: Dynamic.homeCharts_title34
                })
                ...
                var barChart2 = new chart.Chart($(".echarts_bar2"), {
                    type: 'barplotStack',
                    title: Dynamic.homeCharts_title35,
            ...
            if (swiper.activeIndex == 0) {
                chartType = "fBar";
                chartTxt = [Dynamic.analysis_step, Dynamic.analysis_TimeStrp, Dynamic.analysis_sql_attackNum];
            } else if (swiper.activeIndex == 1) {
                chartType = "ipBar";
                chartTxt = [Dynamic.analysis_step, Dynamic.analysis_sql_sqlIp, Dynamic.analysis_sql_attackNum];
            } else if (swiper.activeIndex == 2) {
                chartType = "urlBar";
                chartTxt = [Dynamic.analysis_step, Dynamic.analysis_sql_sqlURL, Dynamic.analysis_sql_attackNum]
            } else if (swiper.activeIndex == 3) {
                chartType = "statusBar";
                chartTxt = [Dynamic.analysis_step,
                    Dynamic.analysis_TimeStrp,
                    Dynamic.analysis_sql_attackStatus1_1,
                    Dynamic.analysis_attackSucPercent,
                    Dynamic.analysis_sql_attackStatus2_1,
                    Dynamic.analysis_attackRedPercent,
                    Dynamic.analysis_sql_attackStatus3_1,
...
function injectionRefresh(data) {
    console.log(data)
    var injectionPvNum = data.injectionPvNum
    var lastInjectionPvNum = data.lastInjectionPvNum

    var injectionIpNum = data.injectionIpNum
    var lastInjectionIpNum = data.lastInjectionIpNum
        //爬取总量
    $(".counterSpan").eq(0).attr("data-value", injectionPvNum);
    //爬取总量环比变化
    var pvNumBt = (injectionPvNum - lastInjectionPvNum) * 100;
    var pl1 = (pvNumBt / lastInjectionPvNum).toFixed(2);
    var pl11 = injectionPvNum - lastInjectionPvNum;
    if (lastInjectionPvNum == 0 && pvNumBt == 0) {
        $(".counterSpan").eq(1).attr("data-value", "--");
        $(".font-red-haze").eq(1).html("")
    } else if (lastInjectionPvNum == 0 && pvNumBt != 0) {
        $(".counterSpan").eq(1).attr("data-value", "∞");
        $(".font-red-haze").eq(1).html("")
    } else {
        if (pl1 < 0) {
    ...
    //爬取源IP总量
    $(".counterSpan").eq(2).attr("data-value", injectionIpNum);
    //爬取源IP总量环比变化
    var ipNumBt = (injectionIpNum - lastInjectionIpNum) * 100;
    var pl2 = (ipNumBt / lastInjectionIpNum).toFixed(2);
    var pl22 = ipNumBt / lastInjectionIpNum;

    if (lastInjectionIpNum == 0 && ipNumBt == 0) {
        $(".counterSpan").eq(3).attr("data-value", "--");
        $(".font-purple-soft").eq(4).html("")
    } else if (lastInjectionIpNum == 0 && ipNumBt != 0) {
        $(".counterSpan").eq(3).attr("data-value", "∞");
        $(".font-purple-soft").eq(4).html("")
    } else {
        if (pl2 < 0) {
